...
const int chipSelect = 4;   // Typically pin 4 or 10 on Arduino Uno
...
void setup() {
  ...
  // Create the header in the CSV file if it's a new file
  File dataFile = SD.open(filename, FILE_WRITE);
  if (dataFile) {
    if (dataFile.size() == 0) {
      dataFile.println("Millis,Temp_C,Pressure_hPa");
    }
    dataFile.close();
  }
  if (!status) {
...
void loop() {
  ...
  if (dataFile) {
    ...
    dataFile.print(",");
    dataFile.println(pressure_event.pressure);
